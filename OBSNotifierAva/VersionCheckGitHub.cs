...
internal sealed class VersionCheckerGitHub : IDisposable
{
    ...
    public delegate Task<MSGDialogResult> MessageBoxDelegate(MSGType type, Dictionary<string, string> customData, Exception? ex);
    ...
    private async void UpdateClient_DownloadStringCompleted(Task<HttpResponseMessage> taskRes)
    {
        var res = taskRes.Result;
        if (!res.IsSuccessStatusCode)
        {
            if (!_isSilentCheck)
                await ShowMessageBox(MSGType.FailedToGetInfo, customData: new Dictionary<string, string> { { "respose", res.StatusCode.ToString() } });
        ...
        try
        {
            ...
            if (newVersion != SkipVersion || !_isSilentCheck)
            {
                ...
                {
                    var updateDialog = await ShowMessageBox(MSGType.InfoUpdateAvailable, new Dictionary<string, string> { { "new_version", newVersion.ToString() }, { "current_version", currentVersion.ToString() } });
    ...
    async Task<MSGDialogResult> ShowMessageBox(MSGType type, Dictionary<string, string>? customData = null, Exception? ex = null)
    {
        if (show_msg_box != null)
        {
            return await Dispatcher.UIThread.InvokeAsync(() => show_msg_box.Invoke(type, customData ?? [], ex));
        }
